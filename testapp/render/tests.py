...
class RenderTextTestCase(TestCase):
    ...
    def test_can_render_image(self):
        ...
        res = self.client.post(
            _url, data=json.dumps(payload), content_type="application/json"
        )
        self.assertEqual(res.status_code, 200)

    def test_can_render_list_image(self):
        _url = reverse("render:list")
        payload = {
            "timestamp": 1526616756,
            "point": "最新 24H 快讯",
            "content": "test",
            "items": [
                "《华尔街日报》调查显示约 19% ICO 存在「误导甚至欺诈」",
                "嘉楠耘智港股融资计划发布",
                "嘉楠耘智港股融资计划发布",
                "纽约金融服务局授 Genesis Global Trading 许可证",
                "以太坊联盟 EEA 发布 CS 1.0 客户规范",
            ],
        }
        res = self.client.post(
            _url, data=json.dumps(payload), content_type="application/json"
